...
FFI_PLUGIN_EXPORT void comaps_set_map_style(int style) {
    LOG(LINFO, ("comaps_set_map_style: style=", style));
    
    if (!g_framework) {
        LOG(LWARNING, ("comaps_set_map_style: Framework not initialized"));
        return;
    }
    
    // Validate style value
    if (style < 0 || style >= MapStyleCount) {
        LOG(LERROR, ("comaps_set_map_style: Invalid style value", style));
        return;
    }
    ...
    LOG(LINFO, ("comaps_set_map_style: Style changed to", style));
}

FFI_PLUGIN_EXPORT int comaps_get_map_style(void) {
    if (!g_framework) {
        LOG(LWARNING, ("comaps_get_map_style: Framework not initialized"));
        return 0; // Return DefaultLight as default
    }
    
    MapStyle currentStyle = g_framework->GetMapStyle();
    LOG(LDEBUG, ("comaps_get_map_style: Current style=", static_cast<int>(currentStyle)));
