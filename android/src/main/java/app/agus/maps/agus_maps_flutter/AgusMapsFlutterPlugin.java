...
public class AgusMapsFlutterPlugin implements FlutterPlugin, MethodCallHandler {
  ...
  public void onMethodCall(@NonNull MethodCall call, @NonNull Result result) {
    ...
    } else if (call.method.equals("checkMapStatus")) {
        Double lat = call.argument("lat");
        Double lon = call.argument("lon");
        if (lat != null && lon != null) {
            int status = nativeCheckMapStatus(lat, lon);
            result.success(status);
        } else {
            result.error("INVALID_ARGUMENT", "lat/lon null", null);
        }
    } else if (call.method.equals("getPlacePageInfo")) {
        String json = nativeGetPlacePageInfo();
        result.success(json); 
    } else if (call.method.equals("buildRoute")) {
        Double lat = call.argument("lat");
        Double lon = call.argument("lon");
        if (lat != null && lon != null) {
            nativeBuildRoute(lat, lon);
            result.success(null);
        }
    } else if (call.method.equals("stopRouting")) {
        nativeStopRouting();
        result.success(null);
    ...
    } else if (call.method.equals("onLocationUpdate")) {
        Double lat = call.argument("lat");
        Double lon = call.argument("lon");
        Double accuracy = call.argument("accuracy");
        if (lat != null && lon != null) {
    ...
    } else if (call.method.equals("getCountryName")) {
        Double lat = call.argument("lat");
        Double lon = call.argument("lon");
        if (lat != null && lon != null) {
  ...
  private native int nativeCheckMapStatus(double lat, double lon);
  // New methods
  private native String nativeGetPlacePageInfo();
  ...
  /**
   * Called from native code when PlacePage status changes (selection).
   * eventType: 0 = Open, 1 = Close
   */
  @SuppressWarnings("unused") // Called from native code
  @Keep
  public void onPlacePageEvent(int eventType) {
      if (mainHandler != null) {
          mainHandler.post(() -> {
              if (channel != null) {
